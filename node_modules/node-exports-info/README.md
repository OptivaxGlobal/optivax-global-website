...
<<<<<<< HEAD
 - `pattern-trailers-no-node-addons`: support for “pattern trailers” was added here, but the `node-addons` condition arrived one minor later (`16.9`)
 - `pattern-trailers`: `pattern-trailers-no-node-addons`, plus the `node-addons` condition (`^14.19 || 16.10 - 16.13`)
=======
