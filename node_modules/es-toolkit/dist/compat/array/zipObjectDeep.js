...
//#region src/compat/array/zipObjectDeep.ts
/**
...
function zipObjectDeep(keys, values) {
	const result = {};
	if (!require_isArrayLike.isArrayLike(keys)) return result;
	if (!require_isArrayLike.isArrayLike(values)) values = [];
	const zipped = require_zip.zip(Array.from(keys), Array.from(values));
	for (let i = 0; i < zipped.length; i++) {
		const [key, value] = zipped[i];
		if (key != null) require_set.set(result, key, value);
	}
	return result;
}
//#endregion
