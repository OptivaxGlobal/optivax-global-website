<<<<<<< HEAD
import { API_VERSION_HEADER_NAME, BASE64URL_REGEX, PKCE_FLOW_ID_PARAM, PKCE_MAX_CONCURRENT_FLOWS, } from './constants';
=======
 ...
 */
export function validatePKCEFlowId(flowId) {
    return typeof flowId === 'string' && PKCE_FLOW_ID_PATTERN.test(flowId) ? flowId : null;
}
export function generatePKCEFlowId() {
...
export const pkceVerifierSlotKey = (storageKey, flowId) => `${storageKey}-flow-${flowId}-code-verifier`;
 ...
 */
export async function storePKCEVerifier(storage, storageKey, flowId, verifier, onEvictFlow) {
 ...
 */
export async function retrievePKCEVerifier(storage, storageKey, flowId) {
    if (flowId) {
 ...
 */
export async function removePKCEVerifier(storage, storageKey, flowId) {
 ...
 */
export async function removeAllPKCEVerifiers(storage, storageKey) {
    const index = await getPKCEFlowIndex(storage, storageKey);
 ...
 */
export function appendFlowIdToRedirectTo(redirectTo, flowId) {
